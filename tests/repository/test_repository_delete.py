...
class TestRepositoryDelete:
    ...
    async def test_delete_by_id(
        ...
        is_deleted = await user_repo.remove_by_id(id_=new_user.id)
    ...
    async def test_delete_by_id_with_wrong_colmun(
        ...
        with pytest.raises(EntityException):
            is_deleted = await user_repo.remove_by_id(
    ...
    async def test_delete_many_repository(
        self, get_test_session, database_provider_test, user_factory
    ):
        user_repo = Repository(UserModel, get_test_session)
        await database_provider_test.init_models()
        ...
        is_deleted = await user_repo.remove_many_by_ids(
    ...
    async def test_delete_many_repository_mix_id(
        ...
        new_user_schema = user_factory()
        new_user = await user_repo.create(data=new_user_schema)
        is_deleted = await user_repo.remove_many_by_ids(ids=[new_user.id, "dummy_id"])
        assert is_deleted == 1
