...
class TestRepositoryUpdate:
    ...
    async def test_update_many_repo(
        self, get_test_session, database_provider_test, user_factory
    ):
        user_repo = Repository(UserModel, get_test_session)
        await database_provider_test.init_models()
        ...
        for user in created_users:
            ...
            count += 1
        updated_users = await user_repo.update_many_by_ids(updates=update_dict)
    ...
    async def test_update_entity(
        ...
        # Creating Fake Users
        user = user_factory()
        created_user = await user_repo.create(data=user)
        LOGGER.info(f"user created: {(created_user.id, created_user.username)}")

        created_user.username = "TakeShot"
        await user_repo.update_entities()

        shot_user = await user_repo.get_one_by_id(id_=created_user.id)
